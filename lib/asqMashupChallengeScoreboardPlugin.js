...
module.exports = ASQPlugin.extend({
  ...
  presenterConnected: coroutine(function *presenterConnectedGen (info){
    ...
    var questionsWithScores = yield this.calculateScoreboard(info.sessionId);
  ...
  afterAnswerSubmission: coroutine(function *answerSubmissionGen (answer){
    ...
    if(question.type !== 'asq-rating') {
      return answer;
    }

    var questionsWithScores = yield this.calculateScoreboard(answer.session);
    ...
    this.asq.socket.emitToRoles('asq:question_type', event,answer.session.toString(), 'ctrl', 'folo')

    //this will be the argument to the next hook
    return answer;
  }),

  calculateScoreboard: coroutine(function *calculateProgressGen(session_id){
    var pipeline = [
      { $match: {
          session: session_id,
        }
    ...
    // var ratingsMap = {};
    var questionIds = ratings.map(function(rating){
      return rating.question;
    });

    var session = yield this.asq.db.model('Session')
      .findById(session_id).exec();
    var presentation = yield this.asq.db.model('Slideshow')
      .findById(session.slides).exec();
    var questions = yield this.asq.db.model('Question')
      .find({_id : {$in: presentation.questions}}).lean().exec();
